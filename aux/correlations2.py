...
weights1 = plot_graph(df1)
idx1 = len(weights1)
weights2 = plot_graph(df2)

# Grafo das metricas
g.es['weight'] = weights1 + weights2
g.es['color'][0:idx1] = ['red' if es['weight'] < 0 else 'blue' for es in g.es]
g.es['color'][idx1:] = ['orange' if es['weight'] < 0 else 'purple' for es in g.es]
widths = []
